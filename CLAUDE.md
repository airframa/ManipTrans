...
**⚠️ CRITICAL BUG FOUND & FIXED AFTER FIRST M4 ATTEMPT — `center_idx`.**
- First M4 run (brush/pan) and a spoon/bowl diagnostic BOTH failed catastrophically: success rate pinned at ~0.0, rollout videos showed hands moving incoherently far from objects. Failure was systemic (every sequence, regardless of difficulty), which bisecting via the easiest sequence (spoon/bowl) confirmed — ruling out task difficulty.
- **Root cause:** `TACODataBase` used `center_idx=None` in the MANO forward pass. This leaves manotorch's `get_rotation_center(beta)` offset (~10cm, mirror-symmetric L/R) UNcompensated before `hand_trans` is added — displacing all TACO MANO joints/vertices ~10cm. TACO's own convention is `center_idx=0` (`hand_trans` IS the wrist world position; see their `hand_pose_loader.py`).
- **Why it hid through M2/M3:** the `center_idx=None` choice was justified as "consistency with `grab_dataset_dexhand.py`" — but GRAB never runs the MANO forward pass (it loads precomputed absolute verts + uses only the fixed J_regressor for joints), so its `center_idx` setting is INERT, never exercised. TACO is the first loader in this repo to actually run MANO forward kinematics with a separate `hand_trans` — so this latent issue only became load-bearing now. Lesson: "match the reference loader" is only valid if the reference actually exercises the code path being copied.
- **How it caused the RL failure:** the ~10cm offset inflated `gt_tips_distance` (fingertip-to-object chamfer, a key observation field) 5–10× (TACO ~7–14cm vs. OakInk-V2/GRAB ~1–3cm). The frozen base imitator was pretrained on tight-contact data, so TACO's "fingers floating far from object" observations were out-of-distribution at nearly every timestep → incoherent output → universal failure. This path is NOT exercised by M1–M3 (loader/retargeting/kinematic checks), which is why everything upstream stayed green.
- **How it was isolated:** ran TACO's OWN reference pipeline (manopth) on the same sequence → 2.33cm (tight, GRAB-like), while ours gave 7.50cm → conclusively our bug, not TACO data quality. Then traced the ~10cm offset directly to `get_rotation_center()`.
- **Fix:** switched `TACODataBase` to `center_idx=0`, adjusted wrist_pos/mano_joints composition to match TACO's convention. **Post-fix validation (Stage 4):** full-sequence-mean `tips_distance` RH 7.50→2.37cm (matches OakInk-V2), LH 6.28→4.87cm (much closer; residual likely genuine TACO looseness — supervisor agrees dataset isn't top-quality). Visual improvement confirmed clearly in replay video. Frame-0-specific values still somewhat elevated (motion-phase artifact — TACO frame 0 is raw/untrimmed approach vs. OakInk-V2's curated mid-grasp start), judged acceptable to resolve via the short diagnostic RL run rather than further static-number debate.
- **Downstream consequence:** the OLD M3 retargeting pkls were fit against displaced targets → invalid. Must regenerate retargeting from scratch (not shift) with the corrected loader. Re-baselining M3 (table + videos + plots) on the corrected pipeline before M4 relaunch.
...
- Estimated cost (from a real prior identical-config run on this hardware, `runs/cross_20aed@0_inspire__06-30-10-25-22/`, OakInk-V2 20aed@0, side=BiH, num_envs=4096): **~15.1 sec/epoch, ~4–4.5 hours wall-clock** for early_stop_epochs=1000, single GPU. That reference run converged to ~0.96–0.98 success rate, reward plateaued ~epoch 700+. **GPU memory measured during the TACO diagnostic run: ~7.7GB / 49GB — huge headroom on the RTX 4090 D (48GB) cards.**
